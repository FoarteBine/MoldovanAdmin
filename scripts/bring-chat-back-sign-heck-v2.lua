...
local function getSignText(container)
    for _, obj in pairs(container:GetDescendants()) do
        if obj:IsA("TextLabel") and obj:IsDescendantOf(workspace) and obj.Parent:IsA("SurfaceGui") then
            if obj:FindFirstAncestor("Sign") then
                return obj.Text, obj
            end
        end
    end
        ...
        if player ~= Players.LocalPlayer and not EXCLUDED_NAMES[player.Name] then
            ...
            for _, container in pairs(containers) do
                if container then
                    ...
                    if remote then
                        if force then
                            task.spawn(function() pcall(function() remote:InvokeServer(text) end) end)
                        else
                            local foundText = nil
